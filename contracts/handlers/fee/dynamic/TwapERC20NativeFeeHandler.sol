 ...
 */
contract TwapERC20NativeFeeHandler is TwapFeeHandler {
    ...
    /**
        ...
        @param depositDataERC20 Data to be passed to ERC20 handler.
        @return fee Returns the fee amount.
        @return tokenAddress Returns the address of the token to be used for fee.
     */
    function _calculateFee(address, uint8, uint8 destinationDomainID, bytes32, bytes calldata depositDataERC20, bytes calldata) internal view override returns (uint256 fee, address tokenAddress) {
        uint256 maxFee = _gasUsed;
        uint256 recipientLength = uint256(bytes32(depositDataERC20[32:64]));
        uint256 pointer = 64 + recipientLength;
        if (depositDataERC20.length > (pointer + 64)) {
            uint256 gas = abi.decode(depositDataERC20[pointer:], (uint256));
            maxFee += gas;
        }
