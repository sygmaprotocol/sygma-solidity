...
module.exports = async function(deployer, network) {
    if (network === "test") {
        ...
        const bridgeInstance = await deployer.deploy(BridgeContract, networksConfig.goerli.domainID, accessControlSegregatorInstance.address);
        const centrifugeAssetInstance = await deployer.deploy(CentrifugeAssetContract);
        const erc20MinterPauserInstance = await deployer.deploy(ERC20MinterPauserContract, networksConfig.goerli.erc20Name, networksConfig.goerli.erc20Symbol);
        const erc20SafeInstance = await deployer.deploy(ERC20SafeContract);
        const erc721MinterBurnerPauserInstance = await deployer.deploy(ERC721MinterBurnerPauserContract, networksConfig.goerli.erc721Name, networksConfig.goerli.erc721Symbol, networksConfig.goerli.erc721URI);
        ...
        // setup ERC20
        await bridgeInstance.adminSetResource(erc20HandlerInstance.address, networksConfig.goerli.erc20ResourceID, erc20MinterPauserInstance.address);
        ...
        // setup ERC721
        await bridgeInstance.adminSetResource(erc721HandlerInstance.address, networksConfig.goerli.erc721ResourceID, erc721MinterBurnerPauserInstance.address);
        ...
        // setup generic
        await bridgeInstance.adminSetGenericResource(genericHandlerInstance.address, networksConfig.goerli.genericResourceID, centrifugeAssetInstance.address, Helpers.blankFunctionSig, Helpers.blankFunctionDepositorOffset, Helpers.getFunctionSignature(centrifugeAssetInstance, 'store'));
