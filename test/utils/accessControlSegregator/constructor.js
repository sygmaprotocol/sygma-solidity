...
contract("AccessControlSegregator - [constructor]", async (accounts) => {
  ...
  it("should replace grant access of deployer if specified in params", async () => {
    ...
    assert.isFalse(
      await accessControlInstance.hasAccess(grantAccessSig, accounts[0])
    );
    assert.isTrue(
      await accessControlInstance.hasAccess(grantAccessSig, accounts[1])
    );
  });
});
