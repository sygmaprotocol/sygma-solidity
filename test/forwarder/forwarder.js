...
contract("Forwarder", async () => {
  ...
  beforeEach(async () => {
    ...
    sign = ethSigUtil.signTypedMessage(relayer1.getPrivateKey(), {
      data: {
        types: types,
        domain: domain,
  ...
  it("If signature is valid, but req.from != signer, it should be reverted and should not be verified", async () => {
    const sign_other = ethSigUtil.signTypedMessage(relayer2.getPrivateKey(), {
      data: {
        types: types,
        domain: domain,
        primaryType: "ForwardRequest",
  ...
  it(`If signature is valid,
      but req.nonce != nonce[signer], it should be reverted and should not be verified`, async () => {
    const request_other = {
      from: relayer1Address,
      to: TestTargetInstance.address,
      value: "0",
    ...
    const sign_other = ethSigUtil.signTypedMessage(relayer1.getPrivateKey(), {
      data: {
        types: types,
        domain: domain,
        primaryType: "ForwardRequest",
  ...
  it("Execute should succeed even if the call to the target failed", async () => {
    ...
    const new_sign = ethSigUtil.signTypedMessage(relayer1.getPrivateKey(), {
      data: {
        types: types,
        domain: domain,
        primaryType: "ForwardRequest",
  ...
  it("Should be failed in case of execute is called with less gas than req.gas", async () => {
    ...
    const new_sign = ethSigUtil.signTypedMessage(relayer3.getPrivateKey(), {
      data: {
        types: types,
        domain: domain,
        primaryType: "ForwardRequest",
    ...
    await TruffleAssert.fails(
      ForwarderInstance.execute(new_request, new_sign, {gas: 100000})
    );
  });
  ...
  it("req.value should be passed to the target contract", async () => {
    ...
    const targetContract_balance = provider.getBalance(
  ...
  it(
    "In case of request is matched with signature, it should not be reverted and nonce should be increased",
    async () => {
    const nonce_before_execute = await ForwarderInstance.getNonce(
      relayer1Address
    );
    await ForwarderInstance.execute(request, sign);
    const nonce_after_execute = await ForwarderInstance.getNonce(
      relayer1Address
    );
    assert.equal(
      nonce_after_execute.toNumber(),
      nonce_before_execute.toNumber() + 1
    );
  });
});
