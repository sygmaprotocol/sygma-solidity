...
contract("E2E ERC20 - Two EVM Chains both with decimal places != 18 with rounding loss", async accounts => {
    ...
    const originDepositAmount = Ethers.utils.parseUnits("1.00000000000000005683", originDecimalPlaces);
    const destinationDepositAmount = Ethers.utils.parseUnits("1", destinationDecimalPlaces);
    const originRelayerConvertedAmount = Ethers.utils.parseUnits("1.000000000000000056", bridgeDefaultDecimalPlaces);
    const destinationRelayerConvertedAmount = Ethers.utils.parseUnits("1", bridgeDefaultDecimalPlaces);
    const roundingLoss = originDepositAmount.sub(Ethers.utils.parseUnits("1", originDecimalPlaces));
    ...
    it(`E2E: depositAmount of Origin ERC20 owned by depositAddress to Destination ERC20
        ...
        TruffleAssert.eventEmitted(originDepositTx, "Deposit", (event) => {
          return (
            ...
            event.handlerResponse === Helpers.toHex(originRelayerConvertedAmount, 32)
          );
        });
        ...
        TruffleAssert.eventEmitted(destinationDepositTx, "Deposit", (event) => {
          return (
            ...
            event.handlerResponse === Helpers.toHex(destinationRelayerConvertedAmount, 32)
          );
        });
        ...
        // Assert ERC20 balance was transferred to recipientAddress minus the roundingLoss
        depositorBalance = await OriginERC20MintableInstance.balanceOf(
          depositorAddress
        );
        assert.strictEqual(depositorBalance.toString(), initialTokenAmount.sub(roundingLoss).toString());
    });
});
