...
contract("BasicFeeHandler - [collectFee]", async (accounts) => {
  ...
  beforeEach(async () => {
    await Promise.all([
      ...
      (ERC20MintableInstance = ERC20MintableContract.new("token", "TOK").then(
        (instance) => (ERC20MintableInstance = instance)
      )),
      (ERC721MintableInstance = ERC721MintableContract.new(
        "ERC721Token",
        "ERC721TOK",
        ""
      ).then((instance) => (ERC721MintableInstance = instance))),
    ]);
    ...
    ERC20BasicFeeHandlerInstance = await BasicFeeHandlerContract.new(
      BridgeInstance.address,
      FeeHandlerRouterInstance.address
    );
    ERC721BasicFeeHandlerInstance = await BasicFeeHandlerContract.new(
      BridgeInstance.address,
      FeeHandlerRouterInstance.address
    );
