...
contract('XC20Handler - [Burn XC20]', async (accounts) => {
    ...
    it('burnableContractAddresses should be marked true in _burnList', async () => {
        ...
        for (i = 0; i < initialResourceIDs.length; i++) {
            await TruffleAssert.passes(BridgeInstance.adminSetResource(XC20HandlerInstance.address, initialResourceIDs[i], initialContractAddresses[i]));
        }

        for (i = 0; i < burnableContractAddresses.length; i++) {
            await TruffleAssert.passes(BridgeInstance.adminSetBurnable(XC20HandlerInstance.address, burnableContractAddresses[i]));
        }

        for (const burnableAddress of burnableContractAddresses) {
            const isBurnable = await XC20HandlerInstance._burnList.call(burnableAddress);
    ...
    it('ERC20MintableInstance2.address should not be marked true in _burnList', async () => {
        ...
        const isBurnable = await XC20HandlerInstance._burnList.call(ERC20MintableInstance2.address);
    ...
    it('ERC20MintableInstance2.address should be marked true in _burnList after setBurnable is called', async () => {
        ...
        await BridgeInstance.adminSetBurnable(XC20HandlerInstance.address, ERC20MintableInstance2.address);
        const isBurnable = await XC20HandlerInstance._burnList.call(ERC20MintableInstance2.address);
        assert.isTrue(isBurnable, "Contract wasn't successfully marked burnable");
    });
});
