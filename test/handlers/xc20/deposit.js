...
contract('XC20Handler - [Deposit ERC20]', async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        await Promise.all([
            ...
            BridgeInstance.adminSetResource(XC20HandlerInstance.address, resourceID, ERC20MintableInstance.address)
        ]);
    ...
    it("When non-contract addresses are whitelisted in the handler, deposits which the addresses are set as a token address will be failed", async () => {
        ...
        await BridgeInstance.adminSetResource(XC20HandlerInstance.address, resourceID_ZERO_Address, ZERO_Address);
        await BridgeInstance.adminSetResource(XC20HandlerInstance.address, resourceID_EOA_Address, EOA_Address);
