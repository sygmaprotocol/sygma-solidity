...
contract('XC20Handler - [setResourceIDAndContractAddress]', async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        await BridgeInstance.adminSetResource(XC20HandlerInstance.address, initialResourceIDs[0], initialContractAddresses[0]);
    });
    ...
    it('new resourceID and corresponding contract address should be set correctly', async () => {
        ...
        await BridgeInstance.adminSetResource(XC20HandlerInstance.address, secondERC20ResourceID, ERC20MintableInstance2.address);
    ...
    it('existing resourceID should be updated correctly with new token contract address', async () => {
        await BridgeInstance.adminSetResource(XC20HandlerInstance.address, initialResourceIDs[0], ERC20MintableInstance1.address);

        const ERC20MintableInstance2 = await ERC20MintableContract.new("token", "TOK");
        await BridgeInstance.adminSetResource(XC20HandlerInstance.address, initialResourceIDs[0], ERC20MintableInstance2.address);
    ...
    it('existing resourceID should be updated correctly with new handler address', async () => {
        ...
        await BridgeInstance.adminSetResource(XC20HandlerInstance2.address, initialResourceIDs[0], ERC20MintableInstance2.address);
    ...
    it('existing resourceID should be replaced by new resourceID in handler', async () => {
        ...
        await BridgeInstance.adminSetResource(XC20HandlerInstance.address, secondERC20ResourceID, ERC20MintableInstance1.address);
