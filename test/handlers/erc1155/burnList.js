...
contract('ERC1155Handler - [Burn ERC1155]', async (accounts) => {
    ...
    it('burnableContractAddresses should be marked true in _burnList', async () => {
        ...
        for (i = 0; i < initialResourceIDs.length; i++) {
            await TruffleAssert.passes(BridgeInstance.adminSetResource(ERC1155HandlerInstance.address, initialResourceIDs[i], initialContractAddresses[i], emptySetResourceData));
        }
