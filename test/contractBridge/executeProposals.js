...
contract("Bridge - [execute proposals]", async (accounts) => {
  ...
  beforeEach(async () => {
    await Promise.all([
      ...
      ERC20MintableContract.new("ERC20token", "ERC20TOK").then(
        (instance) => (ERC20MintableInstance = instance)
      ),
      ERC721MintableContract.new("ERC721token", "ERC721TOK", "").then(
        (instance) => (ERC721MintableInstance = instance)
      ),
      ERC1155MintableContract.new("ERC1155TOK").then(
    ...
    erc20DataHash = Ethers.utils.keccak256(
    ...
    erc721DataHash = Ethers.utils.keccak256(
    ...
    erc1155DataHash = Ethers.utils.keccak256(
      ERC1155HandlerInstance.address + erc1155DepositProposalData.substr(2)
    );
  ...
  it("should create and execute executeProposal successfully", async () => {
    ...
    const recipientERC1155Balance = await ERC1155MintableInstance.balanceOf(
      recipientAddress,
      destinationDomainID
    );
